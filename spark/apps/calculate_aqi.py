...
if __name__ == "__main__":
    ...
    df_with_aqi = df.withColumn("aqi", (df["pollutant_value"] * 100) / 50)
    aqi_counts_df = df_with_aqi.groupBy("pollutant_date").agg(count("*").alias("aqi_count"))

    # aqi_counts_df.writeStream.start()

    query = aqi_counts_df.writeStream.outputMode("update") \
        .foreachBatch(lambda df, epoch_id: write_result(df, epoch_id, "air_quality_index")) \
        .start()
