...
if __name__ == "__main__":
    ...
    avg_of_most_present_pollutant_df = df.groupBy("pollutant_date", "pollutant_parameter") \
                             .agg(avg("pollutant_value").alias("daily_concentration")) \
                             .select("pollutant_date", col("daily_concentration").alias("concentration"))
    
    query = avg_of_most_present_pollutant_df.writeStream.outputMode("update") \
        .foreachBatch(lambda df, epoch_id: write_result(df, epoch_id, "daily_average_of_most_present_pollutant")) \
        .start()
